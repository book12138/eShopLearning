...
namespace eShopLearning.IdentityAuth.Quickstart.UI
{
    /// <summary>
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginInputModel model, string button)
        {
            ...
            {
                string response = Policy.Handle<Exception>() // 定义瞬态故障处理策略
                    .WaitAndRetry( retryCount: 3, sleepDurationProvider: retryAttempt => TimeSpan.FromSeconds(Math.Pow(2, retryAttempt)),
                        onRetry: (exception, timeSpan, retry, ctx) => _logger.LogWarning(exception, "在准备请求用户服务校验用户名密码时，发生错误异常，异常信息为{exceptionMsg}"))
                    .Execute(() => {
                ...
                if (!string.IsNullOrEmpty(response))
                    userServiceResponse = JsonConvert.DeserializeObject<ResponseModel<UserPasswordCheckResultWithUserInfoDto>>(response);
             
                if (userServiceResponse != null && userServiceResponse.Data.IsTrue)
                {
